...
class EpidemicAnalysisService
{
    ...
    1. CLUSTER & COMBINE SIMILAR INDICATIONS:
        If two or more usages target the exact same infectious clinical condition (e.g., "Amoxicillin + Clavulanate for Resistant Infection" and "Cephalosporin for Bacterial Infection" both treat "Bacterial Pharyngitis / Influenza"), you MUST COMBINE their search counts into a single aggregate total.

    2. DOMINANT USAGE EVALUATION & SPIKE DETECTION:
        - Declare an EPIDEMIC WARNING if a single infectious disease/condition reaches 15 or more combined searches AND represents the primary infectious spike in the area, EVEN IF non-infectious chronic usages (like hypertension or diabetes) constitute a large portion of the overall traffic.
        - Do NOT require an infectious condition to exceed 50% of ALL total searches (which include chronic/general medications) to be considered an epidemic. If an infectious condition hits 15+ searches and stands out clearly among infectious/communicable diseases, escalate it immediately.

    3. EVENLY DISTRIBUTED DEMAND (NO PANIC / NO FALSE ALARM):
        - Only declare "Normal" status if search counts for infectious diseases are genuinely low, scattered across unrelated mild conditions, or consist purely of baseline chronic medication searches without any infectious spike reaching the 15-search threshold.

    4. EXCLUDE NON-INFECTIOUS / CHRONIC CONDITIONS:
        - Completely ignore chronic, non-communicable conditions (e.g., Hypertension, Diabetes, Hyperlipidemia, Maintenance Care) when evaluating if a disease is dominating the infectious demand.

    5. MINIMUM VOLUME THRESHOLD:
        - Do NOT issue an epidemic warning if the total combined search count for a specific infectious condition is LESS THAN 15, regardless of its percentage. Treat total counts below 15 as statistically insignificant baseline traffic.
    ...
    You MUST respond with a valid JSON object in pure Modern Standard Arabic (no English words) matching this exact schema:
    {
      ...
      "detected_disease": "اسم المرض المستهدف بعد التجميع باللغة العربية الفصحى أو null",
